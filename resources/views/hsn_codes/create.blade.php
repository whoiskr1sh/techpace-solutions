...
@section('content')
    <div class="max-w-4xl mx-auto px-6 py-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold text-gray-800">Create HSN Code</h2>
        </div>

        <form method="POST" action="{{ route('hsn-codes.store') }}" class="bg-white rounded-lg shadow-sm p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input name="gst_rate" type="number" step="0.01" value="{{ old('gst_rate', 0) }}" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                </div>
            </div>
            ...
            <div class="mb-6 w-full">
                ...
                <textarea name="description" rows="3"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">{{ old('description') }}</textarea>
            </div>
