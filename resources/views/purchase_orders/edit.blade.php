...
@section('content')
    <div class="max-w-7xl mx-auto px-6 py-6">
        ...
        <form method="POST" action="{{ route('purchase-orders.update', $purchase_order) }}" class="bg-white rounded-lg shadow-sm p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input name="po_number" value="{{ old('po_number', $purchase_order->po_number) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="order_date" value="{{ old('order_date', $purchase_order->order_date->toDateString()) }}" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Expected Date (Optional)</label>
                    <input type="date" name="expected_date" value="{{ old('expected_date', optional($purchase_order->expected_date)->toDateString()) }}"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <input name="total_amount" value="{{ old('total_amount', $purchase_order->total_amount) }}" required
            ...
            <div class="mb-6 w-full">
                ...
                <textarea name="notes" rows="3"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">{{ old('notes', $purchase_order->notes) }}</textarea>
            </div>
