...
@section('content')
    <div class="max-w-7xl mx-auto px-6 py-6">
        ...
        <form method="POST" action="{{ route('invoices.update', $invoice) }}" class="bg-white rounded-lg shadow-sm p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">Link Proforma Invoice (optional)</label>
                    <select name="proforma_invoice_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                ...
                <div>
                    ...
                    <select name="sales_order_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                        ...
                        @foreach($salesOrders as $so)
                            <option value="{{ $so->id }}" {{ old('sales_order_id', $invoice->sales_order_id) == $so->id ? 'selected' : '' }}>{{ $so->order_number ?? $so->so_number }} - {{ $so->customer_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input name="customer_name" value="{{ old('customer_name', $invoice->customer_name) }}"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Issue Date</label>
                    <input type="date" name="issue_date" value="{{ old('issue_date', optional($invoice->issue_date)->toDateString()) }}" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                </div>
            ...
            <div class="mb-6">
                ...
                <input name="total_amount" type="number" step="0.01" value="{{ old('total_amount', $invoice->total_amount) }}" required
            ...
            <div class="mb-6 w-full">
                ...
                <textarea name="notes" rows="3"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">{{ old('notes', $invoice->notes) }}</textarea>
            </div>

            <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                <a href="{{ route('invoices.index') }}" class="px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-50 text-sm font-medium border border-gray-300 shadow-sm transition-colors">
                    Cancel
                </a>
