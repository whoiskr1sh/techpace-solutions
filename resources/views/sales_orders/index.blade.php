        ...
        <div class="flex items-center gap-2">
            @can('create', App\Models\SalesOrder::class)
                <a href="{{ route('sales-orders.create') }}"
                    class="inline-flex items-center px-3 py-2 bg-blue-600 text-white rounded">New SO</a>
            @endcan
        </div>
    </div>
            ...
            <tbody>
                @foreach($salesOrders as $so)
                    <tr class="border-t">
                        ...
                        <td class="p-3">{{ $so->quotation?->quotation_number }}
                            <div class="text-xs text-gray-500">{{ $so->quotation?->customer_name }}</div>
                        </td>
                        ...
                        <td class="p-3">
                            <div class="flex items-center gap-2">
                                ...
                                @endcan
                                @can('delete', $so)
                                    <form method="POST" action="{{ route('sales-orders.destroy', $so) }}"
