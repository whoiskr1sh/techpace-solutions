        ...
        <div class="flex items-center gap-2">
            @can('create', App\Models\Quotation::class)
                <a href="{{ route('quotations.create') }}"
                    class="inline-flex items-center px-3 py-2 bg-blue-600 text-white rounded">New Quotation</a>
            @endcan
        </div>
    </div>
            ...
            <tbody>
                @foreach($quotations as $quotation)
                    <tr class="border-t">
                        ...
                        <td class="p-3">{{ $quotation->customer_name }}
                            <div class="text-xs text-gray-500">{{ $quotation->customer_email }}</div>
                        </td>
