...
@section('content')
    <div class="max-w-7xl mx-auto px-6 py-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold text-gray-800">Edit Proforma Invoice</h2>
        </div>

        <form method="POST" action="{{ route('proforma-invoices.update', $proforma_invoice) }}" class="bg-white rounded-lg shadow-sm p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input name="pi_number" value="{{ old('pi_number', $proforma_invoice->pi_number) }}" required
                ...
                <div>
                    ...
                    <select name="sales_order_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                        ...
                        @foreach($salesOrders as $so)
                            <option value="{{ $so->id }}" {{ old('sales_order_id', $proforma_invoice->sales_order_id) == $so->id ? 'selected' : '' }}>{{ $so->order_number ?? $so->so_number }} - {{ $so->customer_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input name="customer_name" value="{{ old('customer_name', $proforma_invoice->customer_name) }}"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Issue Date</label>
                    <input type="date" name="issue_date" value="{{ old('issue_date', optional($proforma_invoice->issue_date)->toDateString()) }}" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">
                </div>
            ...
            <div class="mb-6">
                ...
                <input name="total_amount" type="number" step="0.01" value="{{ old('total_amount', $proforma_invoice->total_amount) }}" required
            ...
            <div class="mb-6 w-full">
                ...
                <textarea name="notes" rows="3"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-3 py-2 border">{{ old('notes', $proforma_invoice->notes) }}</textarea>
            </div>
